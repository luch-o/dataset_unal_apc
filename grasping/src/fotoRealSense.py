...
def find_pose():
    ...
    #    rgbo = rospy.wait_for_message('/camera/rgb/image_color', Image)
        ...
        aus = np.asanyarray(depth_frame.get_data())
        aus2 = np.asanyarray(color_frame.get_data())

        print("hola ",aus.shape)

        fig = plt.figure(figsize=(10, 10))
        ax = fig.add_subplot(1, 2, 1)
        ax.imshow(aus, cmap='gray')
        #for g in grasps:
        #    g.plot(ax)
        ax.set_title('Depth')
        ax.axis('off')
        ...
        frame = hole_filling.process(depth_frame)


        depth_image = np.asanyarray(frame.get_data())
        depth_image1 = depth_image[20:460, 40:600]/8
        color_image = np.asanyarray(color_frame.get_data())


        fig = plt.figure(figsize=(10, 10))
        ax = fig.add_subplot(1, 1, 1)
        ax.imshow(depth_image1, cmap='gray')
        #for g in grasps:
        #    g.plot(ax)
        ax.set_title('DepthAn')
        ax.axis('off')
        plt.show()
        ...
        points_out, ang_out, width_out, depth = predict(depth_image1)
        grasps = grasp.detect_grasps(points_out, ang_out, 0.65, width_img=width_out, no_grasps=3)
        fig = plt.figure(figsize=(10, 10))
